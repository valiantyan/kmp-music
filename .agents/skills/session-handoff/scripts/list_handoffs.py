...
Searches for handoff documents in handoff/ and displays:
- Filename with date
...
def list_handoffs(project_path: str) -> list[dict]:
    ...
    handoffs_dir = Path(project_path) / "handoff"
...
def main():
    ...
    if not handoffs:
        print(f"No handoffs found in {project_path}/handoff/")
        print("\nTo create a handoff, run: python create_handoff.py [task-slug]")
        return

    print(f"Found {len(handoffs)} handoff(s) in {project_path}/handoff/\n")
    print("-" * 80)
