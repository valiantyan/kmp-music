...
苹果平台统一播放迁移实现批次推进到 Ticket 12：macOS AVFoundation bridge 最小真实播放已完成实现、验证、票据更新和交付前审查。
...
- 12 未切换默认桌面 engine，`DesktopAudioRuntimeFactory` 仍使用 `DesktopVlcjAudioPlayerEngine`；未删除 vlcj / LibVLC 链路。

## 下一步

- 当前可继续推进 `13-desktop-default-avfoundation-engine.md`，其前置 12 已满足。
- 后续阻塞链保持不变：14 依赖 11/13；15 依赖 13；16 依赖 14/15；17 依赖 11/14/15/16。
