...
- macOS/Desktop 侧 `DesktopPlaybackSession.kt` 同时承载依赖装配、libVLC runtime 选择、进程生命周期和关闭收口。
...
- 接入 `PlaybackServiceConnector`。
- 接入 `PermissionSettingsOpener`。
- 注入 `nowMillis`。

该 factory 只负责 wiring，不持有进程生命周期状态。后续如果 Android 持久化或 repository 装配调整，只改该 factory。
...
`AndroidPlaybackSession` 委托 runtime 完成 bootstrap、restore 和 controller 访问，不直接管理所有状态字段。

### Media3 command 拆分

`PlaybackMediaCommands.kt` 拆为更小的职责文件。

`AndroidPlaybackMediaButtons` 不作为第四阶段完成态保留。执行拆分时，同一迁移任务内把现有调用点迁移到 catalog、codec、button factory 或 command handler，并删除旧 object。只有遇到明确的 Media3 API 兼容限制时，才允许短期保留转发门面，并在 implementation plan 中写明删除窗口。
...
未 attach actions 时返回 invalid state。按钮刷新命令不在 handler 里直接更新业务状态。
...
承接 `createDesktopPlaybackController`：
...
后续如果 desktop persistence 调整，只动 factory。
...
它不构建具体 vlcj adapter，也不构建 repository 依赖图。
...
Reducer 是纯规则协作者：只接收当前 state 和 adapter event，返回 command loop 可应用的 transition 或 event 列表。它不持有 channel、adapter、ticker 或 coroutine scope，也不直接 mutate `DesktopPlaybackEngineState`。只有 `DesktopPlaybackCommandLoop` 可以应用 reducer result 并改写 runtime state。
...
切歌、暂停、停止、错误和 release 都通过 command loop 停止 ticker。ticker 只向 command loop 投递 `ProgressTick` command，不直接发送 `PlaybackEngineEvent` 或改写 state；command loop 再按当前 generation 发送 position event，避免旧媒体进度污染新媒体。
...
如果新增 Android local unit tests，最终验证还必须运行：
...
如果拆出纯 Kotlin codec 或 command 判断，优先补轻量单元测试：
...
这些测试应放在 `composeApp/src/androidUnitTest`，因为它们依赖 Android `Bundle` 和 Media3 `SessionCommand` / `SessionResult`，不能放进 `commonTest`。若 implementation plan 决定暂不新增 `androidUnitTest`，则上述测试只能作为后续可选项，不能写入本阶段必做验收。
...
- `AndroidPlaybackMediaButtons` 不作为完成态保留；调用点直接依赖新的 catalog、codec、button factory 或 command handler。
...
- `DesktopAdapterEventReducer` 不直接 mutate runtime state，`DesktopProgressTicker` 不直接发送 engine event；状态改写和进度事件发送仍收敛在 command loop。
