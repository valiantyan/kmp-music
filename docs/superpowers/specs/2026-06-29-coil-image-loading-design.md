...
页面层不直接调用 `AsyncImage`、`ImageRequest`、`ImageLoader`、`LocalPlatformContext` 或平台图片解码 API。所有封面显示和封面取色都经由上述门面。旧的 `coverArtPainter` / `rememberPlatformCoverArtPainter` 路径应移除，不保留页面可继续使用的 painter 兼容入口。
...
- 问：兜底资源能否用 `painterResource` 或 `imageResource` 渲染？
  答：不能作为最终图片渲染路径。兜底资源也必须通过 `Res.getUri(...)` 交给 Coil 加载，才能满足“Compose UI 图片加载统一走 Coil”。
- 问：页面能否导入 `LocalPlatformContext` 传给取色函数？
  答：不能。取色函数应在组件层内部取得 Coil 所需上下文，页面只传 `coverArt` 和 `coverImageUri`。
...
- 问：设计是否解决了用户提出的“所有图片加载走 Coil”，而不是只改局部页面？
  答：是。所有 Compose UI 封面显示都经共享门面走 Coil，包含兜底资源；页面不直接依赖平台解码或 Compose resource painter。
...
- 问：Coil 依赖是否会泄漏到每个页面？
  答：不会。页面只调用 `CoverArtImage` 和取色门面，Coil 请求构建和 `LocalPlatformContext` 获取集中在 components 层。
