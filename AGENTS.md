...
- `STRICT` 的 snapshot 必须同时声明只协调的主代理和唯一 writer。writer 在首次实现与首次验证前分别记录执行权里程碑；缺少任一里程碑时禁止 `review`、`render` 或 `complete`。reviewer 必须将里程碑与 Codex 会话轨迹对照：主代理出现仓库探索、文件修改、实现或验证命令时，判定交付失败并要求收口或重新派发。
