...
interface PlaySongUseCase {
    ...
    operator fun invoke(
        song: Song,
        queueSongs: List<Song> = listOf(song),
    ): PlaybackState
}
...
class PlaySongUseCaseImpl(
    ...
    override operator fun invoke(
        ...
        val matchingQueueSongs: List<Song> =
            queueSongs.takeIf { songs: List<Song> ->
        ...
        val nextIndex: Int =
            nextQueueIds.indexOf(element = song.id).takeIf { index: Int ->
                index >= 0
            } ?: 0
        ...
        val nextHistory: List<String> =
            listOf(song.id) +
                currentHistory.filterNot { songId ->
                    songId == song.id
                }
...
class MoveQueueUseCaseImpl(
    ...
    override operator fun invoke(direction: Int): PlaybackState {
        ...
        playbackRepository.saveQueueState(
            state =
                playbackRepository.getQueueState().copy(
                    currentIndex = nextIndex,
                ),
        )
        playbackRepository.savePlaybackState(state = nextState)
