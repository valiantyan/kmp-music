...
class InMemoryPlaybackSnapshotStore : PlaybackSnapshotStore {
     ...
     */
    override suspend fun hasSavedSnapshot(): Boolean = snapshot.playbackState.currentSongId != null || snapshot.queueState.songIds.isNotEmpty()

    /** 返回最近一次保存的队列歌曲 id。 */
    override suspend fun getSavedQueueSongIds(): List<String> = snapshot.queueState.songIds
     ...
     */
    override suspend fun restoreSnapshot(availableSongIds: Set<String>): PlaybackSnapshot = snapshot.filterForRestore(availableSongIds = availableSongIds)
}
...
class RoomPlaybackSnapshotStore(
    ...
    override suspend fun saveSnapshot(snapshot: PlaybackSnapshot) {
        database.withWriteTransaction {
            ...
            database.playbackQueueDao().insertAll(
                items =
                    snapshot.queueState.songIds.mapIndexed { index: Int, songId: String ->
    ...
    override suspend fun getSavedQueueSongIds(): List<String> =
        database.playbackQueueDao().getQueueItems().map { item: PlaybackQueueItemEntity ->
            item.songId
        }
    ...
    override suspend fun getSavedSnapshotIdentity(): PlaybackSnapshotIdentity? {
        val snapshotEntity: PlaybackSnapshotEntity =
            database.playbackSnapshotDao().getSnapshot()
                ?: return null
    ...
    override suspend fun restoreSnapshot(availableSongIds: Set<String>): PlaybackSnapshot {
        ...
        val playbackMode: PlaybackMode =
            PlaybackMode.entries.firstOrNull { mode: PlaybackMode ->
...
private fun PlaybackSnapshot.filterForRestore(availableSongIds: Set<String>): PlaybackSnapshot {
    val filteredSongIds: List<String> =
        queueState.songIds.filter { songId: String ->
    ...
    val restoredIndex: Int =
        filteredSongIds.indexOf(requestedSongId).takeIf { index: Int ->
            index >= 0
