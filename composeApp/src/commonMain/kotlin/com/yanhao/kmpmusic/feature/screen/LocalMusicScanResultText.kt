...
internal fun cancelledScanResultTitle(scanState: LocalMusicScanState.Cancelled): String {
    return CANCELLED_SCAN_TITLE
}

// 取消结果说明保持单一来源，移动端和桌面来源页共同复用。
internal fun cancelledScanResultDetail(scanState: LocalMusicScanState.Cancelled): String {
    return "$CANCELLED_SCAN_MESSAGE · 最近结果：${formatLocalMusicScanDate(timestampMillis = scanState.summary.completedAt)}"
}
