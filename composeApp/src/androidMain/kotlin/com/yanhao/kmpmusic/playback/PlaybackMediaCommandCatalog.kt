...
internal object PlaybackMediaCommandCatalog {
    ...
    private val toggleFavoriteSessionCommand: SessionCommand = SessionCommand(
        CUSTOM_ACTION_TOGGLE_FAVORITE,
        Bundle.EMPTY,
    )

    // 播放模式按钮命令，供 [CommandButton] 和 [MediaSession.Callback] 共享同一个 action。
    private val cycleModeSessionCommand: SessionCommand = SessionCommand(
    ...
    /** 返回收藏按钮命令，供按钮工厂和测试共享。 */
    fun toggleFavoriteCommand(): SessionCommand {
        return toggleFavoriteSessionCommand
    }

    /** 返回播放模式按钮命令，供按钮工厂和测试共享。 */
    fun cycleModeCommand(): SessionCommand {
        return cycleModeSessionCommand
    }
