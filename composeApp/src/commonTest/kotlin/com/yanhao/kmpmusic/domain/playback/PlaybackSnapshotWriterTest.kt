...
class PlaybackSnapshotWriterTest {
     ...
     */
    @Test
    fun progressEventsInsideThrottleWindowAreSkipped(): Unit =
        runTest {
            val repository: InMemoryPlaybackRepository = InMemoryPlaybackRepository()
            val store: RecordingPlaybackSnapshotStore = RecordingPlaybackSnapshotStore()
            var currentTimeMs: Long = 1_000L
     ...
     */
    @Test
    fun nonProgressEventsAreNotThrottled(): Unit =
        runTest {
            val repository: InMemoryPlaybackRepository = InMemoryPlaybackRepository()
            val store: RecordingPlaybackSnapshotStore = RecordingPlaybackSnapshotStore()
            val writer: PlaybackSnapshotWriter =
     ...
     */
    @Test
    fun awaitPendingWritesWaitsForAsyncSaveCompletion(): Unit =
        runTest {
            val repository: InMemoryPlaybackRepository = InMemoryPlaybackRepository()
     ...
     */
    @Test
    fun saveNowAndAwaitPropagatesSnapshotStoreFailure(): Unit =
        runTest {
            val repository: InMemoryPlaybackRepository = InMemoryPlaybackRepository()
            val writer: PlaybackSnapshotWriter =
                PlaybackSnapshotWriter(
    ...
    private class RecordingPlaybackSnapshotStore(
        ...
        /** 只要写过一份快照，就视为存在可恢复数据。 */
        override suspend fun hasSavedSnapshot(): Boolean = savedSnapshots.isNotEmpty()

        /** 返回最近一次保存的队列，供接口契约完整实现。 */
        override suspend fun getSavedQueueSongIds(): List<String> = savedSnapshots.lastOrNull()?.queueState?.songIds ?: emptyList()
        ...
        /** 返回最近一次保存的快照。 */
        override suspend fun restoreSnapshot(availableSongIds: Set<String>): PlaybackSnapshot = savedSnapshots.lastOrNull() ?: PlaybackSnapshot()
    }
