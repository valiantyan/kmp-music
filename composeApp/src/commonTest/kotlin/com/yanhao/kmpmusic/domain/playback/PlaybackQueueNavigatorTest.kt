...
class PlaybackQueueNavigatorTest {
     ...
     */
    @Test
    fun sequenceNextLoopsFromLastToFirst() {
        val result: QueueNavigationResult? =
            navigator.next(
                queueState =
                    QueueState(
     ...
     */
    @Test
    fun sequencePreviousLoopsFromFirstToLast() {
        val result: QueueNavigationResult? =
            navigator.previous(
                queueState =
                    QueueState(
     ...
     */
    @Test
    fun loopOneNextKeepsCurrentWhenRequested() {
        val result: QueueNavigationResult? =
            navigator.next(
                queueState =
                    QueueState(
     ...
     */
    @Test
    fun exactIndexRejectsOutOfRangeIndex() {
        val result: QueueNavigationResult? =
            navigator.exactIndex(
                queueState =
                    QueueState(
                        songIds = listOf("a", "b"),
                        currentIndex = 0,
     ...
     */
    @Test
    fun exactIndexForCurrentShuffleItemDoesNotPolluteHistory() {
        val result: QueueNavigationResult? =
            navigator.exactIndex(
                queueState =
                    QueueState(
     ...
     */
    @Test
    fun engineTransitionUpdatesShuffleHistoryAndRemaining() {
        val result: QueueNavigationResult? =
            navigator.engineTransition(
                queueState =
                    QueueState(
     ...
     */
    @Test
    fun modeChangeResetsShuffleHistoryAndBuildsRemainingForNewMode() {
        val result: QueueState =
            navigator.changePlaybackMode(
                queueState =
                    QueueState(
                        songIds = listOf("a", "b", "c"),
                        currentIndex = 1,
     ...
     */
    @Test
    fun removeCurrentSongSelectsFirstResolvedSongAndResetsShuffleState() {
        val result: QueueNavigationResult? =
            navigator.removeSong(
                queueState =
                    QueueState(
                        songIds = listOf("a", "b", "c", "d"),
                        currentIndex = 2,
     ...
     */
    @Test
    fun removeNonCurrentSongKeepsCurrentSongWhenItStillExists() {
        val result: QueueNavigationResult? =
            navigator.removeSong(
                queueState =
                    QueueState(
     ...
     */
    @Test
    fun recoverableNextTargetRequiresDifferentSongOutsideLoopOne() {
        assertEquals(
            expected = false,
            actual =
                navigator.hasDifferentNextTarget(
                    queueState =
                        QueueState(
                            songIds = listOf("a"),
                            currentIndex = 0,
