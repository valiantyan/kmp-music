...
class PlaybackCoordinatorTest {
     ...
     */
    @Test
    fun playSongUsesWholeCurrentListAsQueue(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val engine = FakeAudioPlayerEngine()
            val coordinator =
     ...
     */
    @Test
    fun playSongFallsBackToSingleSongWhenQueueDoesNotContainTarget(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val coordinator =
                PlaybackCoordinator(
     ...
     */
    @Test
    fun cyclePlaybackModeLoopsThroughThreeModes() {
        val repository = InMemoryPlaybackRepository()
        val coordinator =
            PlaybackCoordinator(
                playbackRepository = repository,
     ...
     */
    @Test
    fun cyclePlaybackModeSyncsPlatformEngineMode() {
        val repository = InMemoryPlaybackRepository()
        val engine = FakeAudioPlayerEngine()
        val coordinator =
            PlaybackCoordinator(
     ...
     */
    @Test
    fun sequenceModeEndedFromLastSongLoopsToFirstSong(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val coordinator =
                PlaybackCoordinator(
     ...
     */
    @Test
    fun loopOneKeepsCurrentSongOnEnded(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val coordinator =
                PlaybackCoordinator(
     ...
     */
    @Test
    fun externalShuffleTransitionUpdatesHistoryAndRemaining(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val coordinator =
                PlaybackCoordinator(
     ...
     */
    @Test
    fun shuffleNextAfterPreviousDoesNotReplayCurrentSong(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val coordinator =
                PlaybackCoordinator(
     ...
     */
    @Test
    fun startCollectsEngineEventsIntoRepository(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val engine = FakeAudioPlayerEngine()
            val coordinator =
     ...
     */
    @Test
    fun playSongPersistsSnapshotBeforeStart(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
     ...
     */
    @Test
    fun restoreSnapshotPrimesEngineForResume(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val snapshotStore = InMemoryPlaybackSnapshotStore()
            val engine = FakeAudioPlayerEngine()
     ...
     */
    @Test
    fun toggleWhileLoadingRequestsPlayInsteadOfPause(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val engine = FakeAudioPlayerEngine()
            val coordinator =
     ...
     */
    @Test
    fun firstProgressEventPersistsSnapshot(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val snapshotStore = InMemoryPlaybackSnapshotStore()
            val engine = FakeAudioPlayerEngine()
     ...
     */
    @Test
    fun explicitPlayAndPauseCommandsUpdateRepositoryThroughEngineEvents(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val engine = FakeAudioPlayerEngine()
            val coordinator =
     ...
     */
    @Test
    fun skipToQueueIndexUsesExactIndexAndRequestedPosition(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val engine = FakeAudioPlayerEngine()
            val coordinator =
     ...
     */
    @Test
    fun nonLoopSingleSongFailureStaysErrorWithoutRetryingSameSong(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val coordinator =
                PlaybackCoordinator(
     ...
     */
    @Test
    fun autoSkipKeepsRecentErrorVisibleUntilPlaybackRecovers(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val coordinator =
                PlaybackCoordinator(
     ...
     */
    @Test
    fun successfulPlaybackResetsFailureCounters(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val coordinator =
                PlaybackCoordinator(
     ...
     */
    @Test
    fun removeCurrentSongKeepsRepositoryAndEngineQueueInSync(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val engine = FakeAudioPlayerEngine()
            val coordinator =
     ...
     */
    @Test
    fun serviceTeardownPersistsFinalPausedSnapshot(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val snapshotStore = InMemoryPlaybackSnapshotStore()
            val coordinator =
     ...
     */
    @Test
    fun processTeardownPropagatesSnapshotSaveFailure(): Unit =
        runTest {
            val repository = InMemoryPlaybackRepository()
            val coordinator =
                PlaybackCoordinator(
    ...
    private class FailingPlaybackSnapshotStore : PlaybackSnapshotStore {
        ...
        override suspend fun saveSnapshot(snapshot: PlaybackSnapshot): Unit = throw IllegalStateException("snapshot write failed")

        /** 测试不需要恢复入口。 */
        override suspend fun hasSavedSnapshot(): Boolean = false

        /** 测试不需要队列入口。 */
        override suspend fun getSavedQueueSongIds(): List<String> = emptyList()

        /** 失败 store 不提供可比较身份。 */
        override suspend fun getSavedSnapshotIdentity(): PlaybackSnapshotIdentity? = null

        /** 测试不需要恢复入口。 */
        override suspend fun restoreSnapshot(availableSongIds: Set<String>): PlaybackSnapshot = PlaybackSnapshot()
    }
}
