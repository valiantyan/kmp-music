...
class PersistentPlaybackRepositoryTest {
     ...
     */
    @Test
    fun savePlaybackStatePreservesActiveRuntimeStatus(): Unit =
        runTest {
     ...
     */
    @Test
    fun newRepositoryRestoresPersistedSnapshotAsPaused(): Unit =
        runTest {
            val fixture: PersistentPlaybackRepositoryFixture = PersistentPlaybackRepositoryFixture()
            val repository: PlaybackRepository = fixture.createRepository()
            repository.savePlaybackState(
     ...
     */
    @Test
    fun saveQueueStateKeepsActiveRuntimePlaybackStatus(): Unit =
        runTest {
            val fixture: PersistentPlaybackRepositoryFixture = PersistentPlaybackRepositoryFixture()
            val repository: PlaybackRepository = fixture.createRepository()
            repository.savePlaybackState(
    ...
    private class FakePlaybackSnapshotDao : PlaybackSnapshotDao {
        ...
        /** 读取当前保存的播放快照。 */
        override suspend fun getSnapshot(): PlaybackSnapshotEntity? = snapshot
    ...
    private class FakePlaybackQueueDao : PlaybackQueueDao {
        ...
        /** 按队列位置读取所有歌曲。 */
        override suspend fun getQueueItems(): List<PlaybackQueueItemEntity> = rows.values.sortedBy { item: PlaybackQueueItemEntity -> item.position }
    ...
    private class FakePlaybackHistoryDao : PlaybackHistoryDao {
        ...
        /** 按位置读取最近播放历史。 */
        override suspend fun getHistoryItems(): List<PlaybackHistoryItemEntity> = rows.values.sortedBy { item: PlaybackHistoryItemEntity -> item.position }
